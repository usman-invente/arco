@extends('Dashboard.Layouts.RTL.layout',['title'=>__('dashboard/animatedSlides.title') ])
...
@section('content')
        ...
        <div class="col-sm-12">
            <div class="panel panel-default card-view">
                <div class="panel-heading">
                    <div class="pull-left">
                        <h6 class="panel-title txt-light">{{__('dashboard/animatedSlides.title_description')}}</h6>
                    </div>
                    <div class="pull-right">
                        <h2><a href="javascript:add();" class="btn-primary btn"><i class="fa fa-plus" ></i> {{__('dashboard/animatedSlides.add_button')}}</a>
                        </h2>
                    </div>
                        ...
                        <div class="table-wrap">
                            <div class="table-responsive" style="color: black;">
                                <table id="datable_1" class="table table-bordered table-striped table-hover  display  pb-30"  >
                                    <thead>
                                    <tr>
                                        <th>{{__('dashboard/animatedSlides.data_table_title_1')}}</th>
                                        <th>{{__('dashboard/animatedSlides.data_table_title_2')}}</th>
                                        <th>{{__('dashboard/animatedSlides.data_table_title_3')}}</th>
                                        <th>{{__('dashboard/animatedSlides.data_table_title_4')}}</th>
                                        <th>{{__('dashboard/animatedSlides.data_table_title_5')}}</th>
                                        <th>{{__('dashboard/animatedSlides.data_table_title_6')}}</th>
                                    </tr>
                                    </thead>
                                    <tbody >
                                    </tbody>
                    ...
                    <div class="modal-dialog" role="document">
                        ...
                        <div class="modal-content"  style="width: 800px;">
                            <div class="modal-header">
                                ...
                                <h5 class="modal-title" id="exampleModalLabel1">{{__('dashboard/animatedSlides.title_description')}}</h5>
                            </div>
                ...
                <div class="modal fade" id="delete_modal" tabindex="-1" role="dialog" aria-labelledby="delete_modalLabel1">
                    <div class="modal-dialog" role="document">

                        <div class="modal-content"  style="width: 800px;">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                <h5 class="modal-title" id="delete_modalLabel1">{{__('dashboard/animatedSlides.title_description')}}</h5>
                            </div>
                            ...
                            <div class="modal-body"  >
                                <div class="form-wrap" style="text-align: center;">
                                    <form id="delete_form" name="delete_form" action="#" method="POST" >
                                        {{ method_field('DELETE') }}
                                        @csrf
                                        <p  style="font-weight: bold; font-size: large; color: black;" >{{__('dashboard/animatedSlides.delete_slide')}}</p>
                                        <br>
                                        <br>
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            function fill_datatable()
            {
                var dataTable = $('#datable_1').DataTable({
                    ...
                    ajax:{
                        url: "{{ route('animatedSlides.index') }}",
            ...
            $('#datable_1 tbody').on( 'click', '.view', function (e) {
                ...
                $.ajax({
                    type: 'GET',
                    url: "{{URL('/dashboard/animatedSlides')}}"+"/"+id,
                    contentType: "html",
                    dataType : 'html',
            ...
            $('#datable_1 tbody').on('click', '.edit', function (e) {
                ...
                $.ajax({
                    type: 'GET',
                    url: "{{URL('/dashboard/animatedSlides')}}" + "/" + id + "/edit",
                    contentType: "html",
                    dataType: 'html',
                    success: function (data) {
            ...
            $('#datable_1 tbody').on( 'click', '.delete', function (e) {
                e.preventDefault();
                $("#delete_modal").modal();
                var id = $(this).attr('id');
                var url = '{{ route("animatedSlides.destroy", ":id") }}';
        ...
        function add() {
            ...
            $.ajax({
                type: 'GET',
                url: "{{URL("/dashboard/animatedSlides/create")}}",
                contentType: "html",
                dataType: 'html',
                success: function (data) {
